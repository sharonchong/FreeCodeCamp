...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round(len(
        df[df["education"] == "Bachelors"]) / len(df) * 100, 1)
    ...
    rich_pct = country_rich / country_total * 100
    highest_earning_country = rich_pct.idxmax()
    highest_earning_country_percentage = round(rich_pct.max(), 1)

    # Identify the most popular occupation for those who earn >50K in India.
    people_IN = df[(df["native-country"] == "India") & (df["salary"] == ">50K")]
    occ_counts = people_IN["occupation"].value_counts()
    top_IN_occupation = occ_counts.idxmax()
